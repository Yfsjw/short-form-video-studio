...
test('stores and returns transcript segments in segment order', async () => {
  const root = await mkdtemp(join(tmpdir(), 'studio-transcript-db-')); const db = new StudioDatabase(join(root, 'studio.db'));
  try {
    ...
    db.createJob(job); db.replaceTranscript(job.id, [{ segmentIndex: 1, startSeconds: 2, endSeconds: 3, text: 'second' }, { segmentIndex: 0, startSeconds: 0, endSeconds: 1, text: 'first' }]);
    assert.deepEqual(db.listTranscript(job.id).map(({ segmentIndex, startSeconds, endSeconds, text }) => ({ segmentIndex, startSeconds, endSeconds, text })), [{ segmentIndex: 0, startSeconds: 0, endSeconds: 1, text: 'first' }, { segmentIndex: 1, startSeconds: 2, endSeconds: 3, text: 'second' }]);
...
test('pipeline integration persists mock-engine segments and cleans temporary audio', async () => {
  ...
  const root = await mkdtemp(join(tmpdir(), 'studio-pipeline-')); const db = new StudioDatabase(join(root, 'studio.db'));
  try {
    const now = new Date().toISOString(); const job: VideoJob = { id: 'job-2', originalFilename: 'source.mp4', storedFilename: 'source.mp4', mimeType: 'video/mp4', sizeBytes: 1, status: 'queued', stage: 'queued', errorMessage: null, durationSeconds: null, width: null, height: null, createdAt: now, updatedAt: now }; db.createJob(job);
    ...
    assert.equal(db.getJob(job.id)?.status, 'completed'); assert.equal(db.listTranscript(job.id)[0]?.text, 'Adapter-provided transcript.');
    assert.equal(db.listClips(job.id)[0]?.candidateId.length, 36);
