...
const schema = z.object({
  ...
  WHISPER_CPP_PATH: z.string().default('whisper-cli'),
  WHISPER_MODEL_PATH: z.string().default('./models/ggml-base.en.bin'),
...
export function loadConfig(env = process.env) {
  ...
  return { ...value, DATABASE_PATH: resolve(value.DATABASE_PATH), UPLOAD_DIR: resolve(value.UPLOAD_DIR), TEMP_DIR: resolve(value.TEMP_DIR), OUTPUT_DIR: resolve(value.OUTPUT_DIR), WHISPER_MODEL_PATH: resolve(value.WHISPER_MODEL_PATH) };
}
