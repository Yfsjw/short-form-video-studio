...
/** Orchestrates real processing stages; future AI providers implement dedicated stage adapters. */
    ...
    try {
      ...
      try {
        ...
        for (const candidate of candidates) {
          const clipId = randomUUID(); const outputFilename = `${clipId}.mp4`; const outputPath = join(jobOutputDir, outputFilename); const now = new Date().toISOString();
          ...
          try {
            const rendered = await this.clipRenderer.render({ sourcePath: join(this.uploadDir, job.storedFilename), startSeconds: candidate.startSeconds, endSeconds: candidate.endSeconds, outputPath });
            this.db.updateClip(clipId, { status: 'completed', durationSeconds: rendered.durationSeconds, errorMessage: null }); completedCount += 1;
          } catch (error) {
        ...
        this.logger.info({ jobId, metadata, transcriptSegmentCount: segments.length, highlightCandidateCount: highlights.length, completedClipCount: completedCount }, 'Video processing and clip extraction completed');
      } finally {
