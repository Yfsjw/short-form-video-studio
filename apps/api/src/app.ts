...
export function createApp(config: AppConfig) {
  ...
  app.get('/test-upload', (_req, res) => {
    res.type('html').send(`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Video Processing Test</title><style>body{font-family:system-ui,sans-serif;max-width:720px;margin:32px auto;padding:0 16px}button{padding:10px 16px;margin-top:12px}pre{white-space:pre-wrap;word-break:break-word;background:#f5f5f5;padding:12px;border-radius:8px}</style></head><body><h1>Video Processing Test</h1><p>Temporary mobile test page.</p><form id="form"><input id="video" name="video" type="file" accept="video/*" required><br><button id="submit" type="submit">Upload video</button></form><pre id="out"></pre><script>const form=document.getElementById('form'),video=document.getElementById('video'),out=document.getElementById('out'),submit=document.getElementById('submit');const readResponse=async r=>{const text=await r.text();if(!text)return {error:{code:'EMPTY_RESPONSE',message:'The server closed the upload request without returning a response. This usually means the upload exceeded the platform/request timeout.'}};try{return JSON.parse(text)}catch{return {error:{code:'NON_JSON_RESPONSE',message:'The server returned an unexpected non-JSON response.',raw:text.slice(0,500)}}};};form.addEventListener('submit',async e=>{e.preventDefault();const file=video.files[0];if(!file)return;submit.disabled=true;out.textContent='Uploading '+(file.size/1024/1024).toFixed(1)+' MB...';const data=new FormData();data.append('video',file);try{const r=await fetch('/api/jobs',{method:'POST',body:data});const j=await readResponse(r);if(!r.ok||j.error){out.textContent=JSON.stringify(j,null,2);return}const id=j.job.id;out.textContent=JSON.stringify(j,null,2);const poll=async()=>{try{const sr=await fetch('/api/jobs/'+id);const sj=await readResponse(sr);out.textContent=JSON.stringify(sj,null,2);if(sj.job&&(sj.job.status==='completed'||sj.job.status==='failed'))return;setTimeout(poll,3000)}catch(err){out.textContent=String(err)}};setTimeout(poll,1000)}catch(err){out.textContent='Upload request failed: '+String(err)+'\n\nIf this happens with a large video, test with a smaller video first; the Render Free proxy can terminate long-running upload requests.'}finally{submit.disabled=false}});</script></body></html>`);
  });
